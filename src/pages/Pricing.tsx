      ...
      <div className="mx-auto max-w-7xl">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              },
              {
                ...
                features: ['Remote support included', '1 on-site health check/month', 'CCTV & ICT diagnostics', 'Monthly status report'],
              },
              {
                name: 'Silver',
                subtitle: 'Standard Cover',
                price: 'From BWP 1,500',
                period: '/ month',
                note: null,
                featured: true,
                tierClass: 'border-hh-red/40 shadow-red-glow',
                badgeClass: 'bg-gradient-to-r from-slate-400 to-slate-600',
                features: ['Remote support included', '2 on-site visits/month', 'Priority 4hr response', 'CCTV + ICT + UPS checks'],
              },
              {
                name: 'Gold',
                ...
                features: ['24/7 monitoring alerts', 'Unlimited remote support', 'Monthly on-site visits', 'Full performance reports', 'Dedicated account manager'],
              },
            ].map((tier) => (
              <GlassCard key={tier.name} className={`p-6 flex flex-col ${tier.tierClass}`}>
                ...
                <Link to="/quote" className={`block text-center text-xs font-medium py-2.5 rounded-md transition-all mt-auto ${tier.featured ? 'bg-gradient-to-r from-hh-red to-red-700 text-white hover:brightness-110' : 'border border-white/10 text-white hover:bg-white/5'}`}>
                  Get This Plan
                </Link>
              </GlassCard>
            ))}
        ...
        <div className="mt-20">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {[
              { name: 'Call-Out', price: 'BWP 250', desc: 'Per visit — ideal for one-off issues', features: ['On-site diagnosis', 'Basic repairs', 'Software fixes'], featured: false },
              { name: 'Monthly Retainer', price: 'BWP 1,500', desc: 'Per month — best for ongoing support', features: ['Priority response', '2 call-outs per month', 'Remote & on-site support', 'Monthly system checkup', '1 System Audit / Report'], featured: true },
              { name: 'Enterprise SLA', price: 'Custom', desc: 'For large offices & businesses', features: ['Dedicated technician', '4-hour response guarantee', 'Full IT management', 'Monthly reporting'], featured: false },
            ].map((p, i) => (
                ...
                <ul className="space-y-2 mb-6">
                  {p.features.map((f) => (
                    <li key={f} className="flex items-center gap-2 text-sm text-white/60">
                      <Check size={14} className="text-hh-red shrink-0" />{f}
                    </li>
                  ))}
                </ul>
                <Link to="/quote" className={`block text-center text-sm font-medium py-2.5 rounded-md transition-all ${i === 1 ? 'bg-gradient-to-r from-hh-red to-red-700 text-white' : 'border border-white/10 text-white hover:bg-white/5'}`}>
                  Get Started
                </Link>
              </GlassCard>
            ))}
